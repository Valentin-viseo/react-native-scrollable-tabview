...
class ScrollableTabView extends Component {
  ...
  _onChangeTab(prevPage, currentPage) {
    ...
    if (
      !!this.contentScrollDistance[currentPage] &&
      this.contentScrollDistance[prevPage] >= this.collapsableBarHeight &&
      this.contentScrollDistance[currentPage] >= this.collapsableBarHeight
    ) {
      setTimeout(() => {
        this.contentView.scrollTo({
          x: 0,
          y: this.contentScrollDistance[currentPage],
          animated: false,
        });
      }, 300);
    } else if (
      ...
      this.contentScrollDistance[currentPage] = this.contentScrollDistance[
        prevPage
      ];
    }
  }
  ...
  render() {
    ...
    if (this.props.txrabBarActiveTextColor) {
    ...
    return (
      <ContainerView
        ...
        onScroll={event => {
          if (event.nativeEvent.contentOffset.y !== this.collapsableBarHeight) {
            this.contentScrollDistance[currentPage] =
              event.nativeEvent.contentOffset.y;
          }

          this.props.onTabScroll(
            event.nativeEvent.contentOffset.y,
            this.collapsableBarHeight
          );
        }}
